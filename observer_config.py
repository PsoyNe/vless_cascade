...
FAILURES_TO_SWITCH = 3            # Отказов подряд для переключения
...
# -------------------- ДВОЙНАЯ ПРОВЕРКА РЕЗЕРВА --------------------
# Сколько раз проверять ссылку перед добавлением в резерв.
#
# Если 1 — проверка один раз (старое поведение).
# Если 2 и больше — проверяем N раз с паузой BACKUP_CHECK_INTERVAL между
# попытками. В резерв попадают только те ссылки, которые ВСЕ N проверок
# прошли успешно.
#
# Зачем: одна успешная проверка может быть случайной. Ссылка, которая
# отвечает 1 раз из 2, не должна попадать в резерв.
BACKUP_CHECK_ATTEMPTS = 2

# Пауза между проверками при двойной проверке (сек).
